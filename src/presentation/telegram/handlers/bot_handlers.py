...
class TelegramBotHandlers:
    ...
    async def on_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if session.state in {ConversationState.AWAIT_QUOTES_GROUP, ConversationState.AWAIT_QUOTES_CALENDAR}:
    ...
    async def _handle_best_group_callback(self, telegram_user_id: int, query, data: str) -> None:
        ...
        await query.answer()
        if query.message is not None:
            await query.message.reply_text(response, reply_markup=build_main_menu_keyboard())
    ...
    async def _handle_available_category_callback(self, telegram_user_id: int, query, data: str) -> None:
        ...
        run_id, rows = self._adapter.get_category_matches(guest_id=guest_id, category_name=category_name)
        await query.answer()
        if query.message is not None:
            await query.message.reply_text(
                self._render_category_details(category_name=category_name, run_id=run_id, rows=rows),
    ...
    def _render_category_details(self, *, category_name: str, run_id: str, rows: list[MatchedDateRecord]) -> str:
        if not rows:
            return f"{category_name}\nНет данных."
        lines = [f"Категория: {category_name}", f"run_id: {run_id}"]
        for r in rows[:50]:
            period_end = r.period_end or r.date
            old_rub = r.old_price_minor / 100
            new_rub = r.new_price_minor / 100
            offer = r.offer_title or r.offer_id or "—"
            lines.append(f"{r.date.isoformat()} - {period_end.isoformat()} | {r.tariff} | {old_rub:.2f} -> {new_rub:.2f} RUB | offer: {offer}")
        return "\n".join(lines)
