...
class TelegramUseCasesAdapter:
    ...
    def recalculate_matches(self, *, date_from: date | None = None, date_to: date | None = None, booking_date: date | None = None) -> str:
        today = date.today()
        ...
        results = use_case.execute(date_from=actual_date_from, date_to=actual_date_to, booking_date=actual_booking_date)
    ...
    def get_available_categories(self, *, guest_id: str) -> list[str]:
        run_id = self._matches_run_repo.get_latest_run_id()
        if not run_id:
            return []
        rows = self._matches_run_repo.get_run_rows(run_id)
