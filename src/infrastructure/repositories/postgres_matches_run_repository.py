...
class PostgresMatchesRunRepository(MatchesRunRepository):
    ...
    def _init_schema(engine: Engine) -> None:
        ...
        with engine.begin() as conn:
            conn.execute(text("ALTER TABLE matches_run ADD COLUMN IF NOT EXISTS snapshot_id TEXT"))
            conn.execute(
                text(
                    """
            ...
            conn.execute(text("CREATE INDEX IF NOT EXISTS ix_matches_run_guest_period ON matches_run(guest_id, period_label);"))
            conn.execute(text("CREATE INDEX IF NOT EXISTS ix_matches_run_snapshot ON matches_run(snapshot_id);"))

    def replace_run(self, run_id: str, rows: list[MatchedDateRecord]) -> None:
        snapshot_id = run_id
        logger.info("matches_run_snapshot_start snapshot_id=%s rows=%s", snapshot_id, len(rows))
        ...
        logger.info("matches_run_snapshot_published snapshot_id=%s", snapshot_id)
    ...
    def get_latest_run_id(self) -> str | None:
        ...
        stmt = select(matches_run_table.c.run_id).order_by(matches_run_table.c.computed_at.desc()).limit(1)
        with self._engine.connect() as conn:
